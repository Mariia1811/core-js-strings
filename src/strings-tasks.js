...
function findLongestWord(sentence) {
  ...
  const longest = words.reduce((long, currentWord) => {
    return currentWord.length > long.length ? currentWord : long;
  }, '');
  return longest;
}
...
function reverseWords(str) {
  const x = str.split(' ').map((el) => el.split('').reverse().join(''));
  return x.join(' ');
}
...
function encodeToRot13(str) {
  return str.replace(/[a-zA-Z]/g, (char) => {
    const offset = char.toUpperCase() <= 'M' ? 13 : -13;
